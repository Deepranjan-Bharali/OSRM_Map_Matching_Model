...
# Ensure the full_trajectory_df is loaded (in case the script was stopped and restarted).
# If you just ran the previous sections, full_trajectory_df will already be in memory.
# If you comment out previous sections for testing only this part, uncomment the line below:
# full_trajectory_df = pd.read_csv(os.path.join(trajectory_output_dir, "simulated_gps_trajectories.csv"))

# Make sure 'timestamp' column is in datetime format, especially if loaded from CSV
if not pd.api.types.is_datetime64_any_dtype(full_trajectory_df['timestamp']):
    full_trajectory_df['timestamp'] = pd.to_datetime(full_trajectory_df['timestamp'])

# Convert GeoDataFrames for highways and service roads to use for spatial queries.
# Ensure they have an appropriate CRS (Coordinate Reference System).
# EPSG:4326 is WGS84, common for latitude/longitude.
gdf_highways_geo = gdf_highways.to_crs(epsg=4326) # Ensure it's in lat/lon CRS
gdf_service_roads_geo = gdf_service_roads.to_crs(epsg=4326) # Ensure it's in lat/lon CRS

# Basic check: are our road GeoDataFrames populated?
if gdf_highways_geo.empty:
    print("WARNING: gdf_highways_geo is empty. No highway features will be calculated.")
if gdf_service_roads_geo.empty:
    print("WARNING: gdf_service_roads_geo is empty. No service road features will be calculated.")


# --- Feature Type 1: Kinematic Features (from GPS trajectory itself) ---
# These describe the movement characteristics of the vehicle.

print("Extracting kinematic features (speed, acceleration, turning)...")

# Calculate time difference between consecutive points within each trajectory
full_trajectory_df['time_diff_seconds'] = full_trajectory_df.groupby('trajectory_id')['timestamp'].diff().dt.total_seconds().fillna(0)

# Speed is already 'speed_kph'. Let's add speed in meters/second for consistency with acceleration.
full_trajectory_df['speed_mps'] = full_trajectory_df['speed_kph'] * 1000 / 3600

# Calculate instantaneous acceleration (change in speed over time).
...
full_trajectory_df['acceleration_mps2'] = full_trajectory_df['acceleration_mps2'].replace([np.inf, -np.inf], 0).fillna(0) # Replace Inf/-Inf with 0

# Calculate turning rate/angular velocity (change in heading over time).
full_trajectory_df['heading_diff'] = full_trajectory_df.groupby('trajectory_id')['heading'].diff().fillna(0)
# Normalize heading differences to -180 to 180 degrees (important for circular values)
...
# --- Feature Type 2: Geometric Features (Spatial relationship to map data) ---
# These describe how close a GPS point is to different types of roads.

print("Extracting geometric features (distance to nearest highway/service road)... This might take a moment.")

# Convert the Pandas DataFrame of GPS points into a GeoDataFrame.
# This is necessary to perform spatial operations like distance calculations.
gdf_trajectories = gpd.GeoDataFrame(
    full_trajectory_df,
    geometry=gpd.points_from_xy(full_trajectory_df.longitude, full_trajectory_df.latitude),
    crs="EPSG:4326" # Ensure CRS matches map data (lat/lon)
)

# Initialize columns to store the calculated distances.
gdf_trajectories['dist_to_nearest_highway'] = np.nan
gdf_trajectories['dist_to_nearest_service_road'] = np.nan

# Calculate distance to nearest highway segment for each GPS point
if not gdf_highways_geo.empty and not gdf_trajectories.empty:
    # Increased max_distance to ensure finding a match even with noise
    sjoined_highway = gpd.sjoin_nearest(gdf_trajectories, gdf_highways_geo, how="left",
                                        max_distance=0.1, # Increased search radius (approx 11 km)
                                        distance_col="distance_sjoin_highway_deg")

    # Check if 'index_right' column exists in the sjoined result
    if 'index_right' in sjoined_highway.columns and not sjoined_highway.empty:
        # Loop through the sjoined results to get precise point-to-line distances.
        for idx, row in sjoined_highway.iterrows():
            if pd.notna(row['index_right']): # Check if a nearest highway segment was found for this row
                point_geom = row.geometry
                # Use .loc[row['index_right']] for safe access to the original gdf_highways_geo
                line_geom = gdf_highways_geo.loc[row['index_right']].geometry

                # Project the point onto the line and interpolate to get the closest point ON the line.
                closest_point_on_line = line_geom.interpolate(line_geom.project(point_geom))

                # Calculate the distance between the GPS point and the closest point on the line.
                # Multiply by ~111,000 to convert degrees to approximate meters.
                gdf_trajectories.loc[idx, 'dist_to_nearest_highway'] = point_geom.distance(closest_point_on_line) * 111000
    else:
        print("INFO: No nearest highway segments found within max_distance, or sjoined_highway is empty/missing index_right.")
else:
    print("WARNING: Skipping highway distance calculation. Either gdf_highways_geo or gdf_trajectories is empty.")


# Calculate distance to nearest service road segment for each GPS point (same logic as above)
if not gdf_service_roads_geo.empty and not gdf_trajectories.empty:
    sjoined_service = gpd.sjoin_nearest(gdf_trajectories, gdf_service_roads_geo, how="left",
                                        max_distance=0.1, # Increased search radius
                                        distance_col="distance_sjoin_service_deg")

    if 'index_right' in sjoined_service.columns and not sjoined_service.empty:
        for idx, row in sjoined_service.iterrows():
            if pd.notna(row['index_right']):
                point_geom = row.geometry
                line_geom = gdf_service_roads_geo.loc[row['index_right']].geometry
                closest_point_on_line = line_geom.interpolate(line_geom.project(point_geom))
                gdf_trajectories.loc[idx, 'dist_to_nearest_service_road'] = point_geom.distance(closest_point_on_line) * 111000
    else:
        print("INFO: No nearest service road segments found within max_distance, or sjoined_service is empty/missing index_right.")
else:
    print("WARNING: Skipping service road distance calculation. Either gdf_service_roads_geo or gdf_trajectories is empty.")


# Fill any NaN (Not a Number) values in distances with a large number (e.g., 9999 meters).
# This indicates that no road of that type was found within the search radius.
gdf_trajectories['dist_to_nearest_highway'].fillna(9999, inplace=True)
gdf_trajectories['dist_to_nearest_service_road'].fillna(9999, inplace=True)


# --- Feature Type 3: Combined/Ratio Features ---
# These combine existing features to create more powerful ones.

# Create a 'distance_ratio' feature. This is very powerful for parallel roads.
# A small ratio means closer to service road, large ratio means closer to highway.
# Add a tiny number (1e-6) to the denominator to prevent division by zero if distance is 0.
...
# Replace any infinite values (if dist_to_nearest_highway was 0 and service was not) with a large number
gdf_trajectories['dist_ratio_service_highway'].replace([np.inf, -np.inf], 1e6, inplace=True)
...
if selected_segment_for_matching is None:
    ...
    if not gdf_highways_geo.empty and not gdf_new_trajectory_points.empty:
        sjoined_highway_new = gpd.sjoin_nearest(gdf_new_trajectory_points, gdf_highways_geo, how="left",
        ...
        print("WARNING: Skipping highway distance calculation for new trajectory. Either gdf_highways_geo or gdf_new_trajectory_points is empty.")

    if not gdf_service_roads_geo.empty and not gdf_new_trajectory_points.empty:
        sjoined_service_new = gpd.sjoin_nearest(gdf_new_trajectory_points, gdf_service_roads_geo, how="left",
        ...
        print("WARNING: Skipping service road distance calculation for new trajectory. Either gdf_service_roads_geo or gdf_new_trajectory_points is empty.")
